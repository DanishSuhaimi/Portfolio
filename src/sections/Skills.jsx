import { ChevronLeft, ChevronRight, Quote } from "lucide-react";
import { useState } from "react";

// Dynamic imports to prevent 404 errors in production
import programmingPng from "../assets/programming.png";
import databasePng from "../assets/database.png";
import analyticsPng from "../assets/analytics.png";
import toolsPng from "../assets/tools.png";

const skills = [
  {
    quote: (
      <>
        <strong>Basic:</strong> C, C++, SQL, JSON, R
        <br />
        <strong>Intermediate:</strong> PHP, HTML, Java, JavaScript, Python, CSS
      </>
    ),
    author: "Programming Languages",
    role: "Core Development Skills",
    avatar: programmingPng, // Use imported module reference
  },
  {
    quote: (
      <>
        <strong>Basic:</strong> MySQL, Hadoop, Hive, Firebase
      </>
    ),
    author: "Databases",
    role: "Data Storage & Management",
    avatar: databasePng, // Use imported module reference
  },
  {
    quote: (
      <>
        <strong>Basic:</strong> Tableau
        <br />
        <strong>Intermediate:</strong> Power BI
      </>
    ),
    author: "Data Analytics & Visualization",
    role: "Analytical Visualization",
    avatar: analyticsPng, // Use imported module reference
  },
  {
    quote: (
      <>
        <strong>Basic:</strong> phpMyAdmin, NetBeans, RStudio
        <br />
        <strong>Intermediate:</strong> Android Studio, Visual Studio Code, Canva, Jupyter Notebook, Microsoft Office 365, XAMPP, Google Colab
      </>
    ),
    author: "Tools & Platforms",
    role: "Development Tools",
    avatar: toolsPng, // Use imported module reference
  },
];

export const Skills = () => {
  const [activeIdx, setActiveIdx] = useState(0);

  const next = () => {
    setActiveIdx((prev) => (prev + 1) % skills.length);
  };

  const previous = () => {
    setActiveIdx(
      (prev) => (prev - 1 + skills.length) % skills.length
    );
  };

  return (
    <section id="skills" className="py-32 relative overflow-hidden">
      <div
        className="absolute top-1/2 left-1/2
       w-[800px] h-[800px] bg-primary/5
        rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2"
      />

      <div className="container mx-auto px-6 relative z-10">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-secondary-foreground text-sm font-medium tracking-wider uppercase animate-fade-in">
            Technical Skills
          </span>

          <h2 className="text-4xl md:text-5xl font-bold mt-4 mb-6 animate-fade-in animation-delay-100 text-secondary-foreground">
            Capabilities{" "}
            <span className="font-serif italic font-normal text-white">
              that drive successful projects.
            </span>
          </h2>
        </div>

        {/* Skills Carousel */}
        <div className="max-w-4xl mx-auto">
          <div className="relative">
            {/* Main Skills */}
            <div className="glass p-8 rounded-3xl md:p-12 glow-border animate-fade-in animation-delay-200">
              <div className="absolute -top-4 left-8 w-12 h-12 rounded-full bg-primary flex items-center justify-center">
                <Quote className="w-6 h-6 text-primary-foreground" />
              </div>

              <blockquote className="text-xl md:text-2xl font-medium leading-relaxed mb-8 pt-4">
                {skills[activeIdx].quote}
              </blockquote>

              <div className="flex items-center gap-4">
                <img
                  src={skills[activeIdx].avatar}
                  alt={skills[activeIdx].author}
                  className="w-14 h-14 rounded-full object-cover ring-2 ring-primary/20"
                />

                <div>
                  <div className="font-semibold">
                    {skills[activeIdx].author}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {skills[activeIdx].role}
                  </div>
                </div>
              </div>
            </div>

            {/* Skills Navigation */}
            <div className="flex items-center justify-center gap-4 mt-8">
              <button
                className="p-3 rounded-full glass hover:bg-primary/10 hover:text-primary transition-all"
                onClick={previous}
              >
                <ChevronLeft />
              </button>

              <div className="flex gap-2">
                {skills.map((_, idx) => (
                  <button
                    key={idx}
                    onClick={() => setActiveIdx(idx)}
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      idx === activeIdx
                        ? "w-8 bg-primary"
                        : "bg-muted-foreground/30 hover:bg-muted-foreground/50"
                    }`}
                  />
                ))}
              </div>

              <button
                onClick={next}
                className="p-3 rounded-full glass hover:bg-primary/10 hover:text-primary transition-all"
              >
                <ChevronRight />
              </button>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};